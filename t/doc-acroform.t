...
lives-ok {$doc = PDF::Doc.open("t/pdf/samples/OoPdfFormExample.pdf")}, "open form example  lives";

my $cat = $doc.Root;
isa-ok $cat, ::('PDF::Doc::Type::Catalog'), 'document root';

my $acroform = $cat.AcroForm;
does-ok $cat.AcroForm, ::('PDF::Doc::Type::AcroForm');
...
does-ok @fields[0], ::('PDF::Doc::Type::Field'), '.Fields';
isa-ok @fields[0], ::('PDF::Doc::Type::Annot::Widget'), 'field type';

is @fields[0].Type, 'Annot', 'Type';
is @fields[0].Subtype, 'Widget', 'Subtype';
is @fields[0].F, 4, '.F';
is @fields[0].FT, 'Tx', '.FT';
isa-ok @fields[0]<P>, ::('PDF::Doc::Type::Page'), '<P>';
my $page = @fields[0].P;
isa-ok $page, ::('PDF::Doc::Type::Page'), '.P';
...
does-ok $appearance, ::('PDF::Doc::Type::Appearance'), '.AP';
isa-ok $appearance.N, ::('PDF::Doc::Type::XObject::Form'), '.AP.N';
ok $page.Annots[0] === @fields[0], 'first field via page-1 annots';

my $country = @fields[5];
does-ok $country, ::('PDF::Doc::Type::Field::Choice'), 'choice field';
is +$country.Opt, 28, 'choice options';
is $country.Opt[0], 'Austria', 'choice first option';

my $languages = @fields[8];
does-ok $languages, ::('PDF::Doc::Type::Field::Button'), 'Button field';
$appearance = $languages.AP;
does-ok $appearance, ::('PDF::Doc::Type::Appearance'), '.AP';
isa-ok $appearance.N.Yes, ::('PDF::Doc::Type::XObject::Form'), '.AP.N.Yes';
