...
role PDF::DOM::Type::Field::Choice
    ...
    my subset Options of Array where { !.first( none(*.isa(Array), *.isa(PDF::DOM::Object::TextString) ) ) }
    sub coerce(Array $fields, Options) {

        for $fields.keys {
	    given $fields[$_] {
		when Options { &?ROUTINE($_) }
		default { PDF::DOM::Type::Field.coerce($_, PDF::DOM::Object::TextString)}
	    }
	}
    }

    has Options $.Opt is entry( :&coerce );    #| (Optional) An array of options to be presented to the user. Each element of the array is either a text string representing one of the available options or an array consisting of two text strings: the option’s export value and the text to be displayed as the name of the option
