...
use PDF::Doc::Contents::Text::Line;
use PDF::Doc::Contents::Text::Atom;
use PDF::Doc::Op :OpNames;

class PDF::Doc::Contents::Text::Block {
	...
	$!font-base-height = $font.height( $!font-size, :from-baseline );
        ...
        while @chunks {
            ...
            my $atom = PDF::Doc::Contents::Text::Atom.new( |%atom );
            if $kerning {
                $atom.sticky = True;
            }
    ...
    multi submethod BUILD(:@atoms!,
        ...
        my PDF::Doc::Contents::Text::Line $line;
	...
	@atoms = @atoms;
		...
		if $!horiz-scaling != 100;
            ...
            if !$line || ($!width && $line.atoms && $visual-width > $!width) {
                ...
                $line = PDF::Doc::Contents::Text::Line.new();
                $line-width = 0.0;
		$char-count = 0;
                @!lines.push: $line;
            }
