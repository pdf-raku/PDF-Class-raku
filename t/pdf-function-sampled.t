...
given  $function-obj.calculator {
    is-result .evaluate([0]), [0, 17/255, 33/255, 48/255];
    is-result .evaluate([1]), [1, 1, 1, 160/255];
    is-result .evaluate([.5]), [1/2, 136/255, 144/255, 104/255];
    is-result .evaluate([.25]), [1/4, 3/10, 88.5/255, 76/255];
}
...
given  $function-obj.calculator {
    is-result .evaluate([0]), [0, 17/255, 33/255, 48/255];
    is-result .evaluate([1]), [1, 1, 1, 160/255];
    is-result .evaluate([.5]), [1, 1, 1, 160/255];
    is-result .evaluate([.25]), [1/2, 136/255, 144/255, 104/255];
}

$function-obj.Encode = [0, .8];

is-result $function-obj.evaluate([.5]), [0.400000, 0.440000, 0.477647, 0.363922];
is-result $function-obj.evaluate([1]), [0.800000, 0.813333, 0.825882, 0.539608];

$function-obj.Encode = [0, 1];
$function-obj.Range = [0, 1, -1, 1, 0, 2, 1, 2];

given  $function-obj.calculator {
    is-result .evaluate([0]), [0, -221/255, 66/255, 303/255];
    is-result .evaluate([.5]), [1/2, 17/255, 288/255, 359/255];
    is-result .evaluate([1]), [1, 1, 2, 415/255];
    is-result .evaluate([.25]), [1/4, -102/255, 177/255, 331/255];
}
...
given  $function-obj.calculator {
    is-result .evaluate([0]), [0, 0, 66/255, 303/255];
    is-result .evaluate([.5]), [1/2, 17/255, 1, 359/255];
    is-result .evaluate([1]), [1, 1, 1, 415/255];
    is-result .evaluate([.25]), [1/4, 0, 177/255, 331/255];
}
...
given  $function-obj.calculator {
    is-result .evaluate([0]), [0, 17/255, 33/255, 48/255];
    is-result .evaluate([1]), [161/255, 162/255, 163/255, 164/255];
    is-result .evaluate([.5]), [80.5/255, 89.5/255, 98/255, 106/255];
    is-result .evaluate([.25]), [.157843, 0.208824, 0.256863, 0.301961,];
...
given $function-obj.calculator {
    is-result .evaluate([0, 0]), [0, 0, 0, 0], 'mag-yel(0,0)';
    is-result .evaluate([1, 1]), [0, 1, 2/3, 0], 'mag-yel(1,1)';
    is-result .evaluate([1, .5]), [0, 1, 1/3, 0], 'mag-yel(1,.5)';
    is-result .evaluate([.5, .25]), [0, 1/2, 1/6, 0], 'mag-yel(.5,.25)';
}
...
given $function-obj.calculator {
    is-result .evaluate([0, 0, 0]), [0, 0, 0, 0], 'cyan-mag-yel(0,0,0)';
    is-result .evaluate([1, 1, 1]), [1/3, 1, 2/3, 0], 'cyan, mag-yel(1,1,1)';
    is-result .evaluate([1, 1/3, .5]), [1/3, 1/3, 1/3, 0], 'cyan, mag-yel(1,1/3,.5)';
