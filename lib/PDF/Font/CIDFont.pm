...
##    use PDF::CIDSystemInfo; # todo - causing failures in pdf-font-cidfont.t
    ...
    has CIDFontDescriptor $.FontDescriptor is entry(:required, :indirect); #| (Required; must be an indirect reference) A font descriptor describing the CIDFont’s default metrics other than its glyph widths
    ...
    has Numeric @.DW2 is entry(:len(2));                               #| (Optional; applies only to CIDFonts used for vertical writing) An array of two numbers specifying the default metrics for vertical writing
