...
class PDF::DOM::Type::Font::Type3
    ...
    has Array $.FontBBox is entry(:required);           #| (Required) A rectangle (see Section 3.8.4, “Rectangles”) expressed in the glyph coordinate system, specifying the font bounding box.
    has Array $.FontMatrix is entry(:required);         #| (Required) An array of six numbers specifying the font matrix, mapping glyph space to text space
    ...
    has Int $.LastChar is entry(:required);             #| (Required) The last character code defined in the font’s Widths array

    has Array $.Widths is entry(:required);             #| (Required) An array of (LastChar − FirstChar + 1) widths, each element being the glyph width for the character code that equals FirstChar plus the array index.
