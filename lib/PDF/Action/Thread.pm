...
class PDF::Action::Thread
    ...
    multi sub coerce(Hash $_, ThreadLike) { PDF::COS.coerce($_, PDF::Thread) }
    multi sub coerce(Str  $_, ThreadLike) { PDF::COS.coerce($_, PDF::COS::TextString) }
    multi sub coerce($_, ThreadLike) is default { fail "unable to coerce {.perl} to a Thread" }
    ...
    multi sub coerce(Hash $_, BeadLike) { PDF::COS.coerce($_, PDF::Bead) }
