...
subset PDF::Struct::Action::Destination of PDF::DAO where DestinationArray | PDF::Struct::Action; #| e.g. for Catalog /OpenAction entry
...
role PDF::Struct::Action
    ...
    multi sub coerce(Hash $dict is rw where { .<S> ~~ ActionSubtype }, PDF::Struct::Action) {
	my $subclass = PDF::DAO.delegator.find-delegate( 'Action', $dict<S> );
	PDF::DAO.coerce($dict, $subclass);
    }

    multi sub coerce(Hash $dict is rw, PDF::Struct::Action) is default {
	warn "unknown action subtype: $dict<S>"
	    if $dict<S>:exists;
	PDF::DAO.coerce($dict, PDF::Struct::Action);
    }

    multi sub coerce(Hash $dict is rw, PDF::Struct::Action::Destination) {
	coerce( $dict, PDF::Struct::Action );
    }

    my subset NextActionArray of Array where { !.first( !*.isa(PDF::Struct::Action) ) }
    my subset NextAction of Any where PDF::Struct::Action | NextActionArray;

    multi sub coerce(Hash $actions, NextAction) {
      PDF::DAO.coerce( $_, PDF::Struct::Action )
    }
    multi sub coerce(Array $actions, NextAction) {
      PDF::DAO.coerce( $actions[$_], PDF::Struct::Action )
	  for $actions.keys;
    }
