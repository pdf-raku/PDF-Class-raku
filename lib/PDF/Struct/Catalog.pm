...
class PDF::Struct::Catalog
    ...
    has PDF::Struct::Pages $.Pages is entry(:required, :indirect);
    ...
    has PDF::Struct::ViewerPreferences $.ViewerPreferences is entry;  #| (Optional; PDF 1.2) A viewer preferences dictionary specifying the way the document is to be displayed on the screen.
    ...
    use PDF::Struct::Outlines;
    has PDF::Struct::Outlines $.Outlines is entry(:indirect); #| (Optional; must be an indirect reference) The outline dictionary that is the root of the document’s outline hierarchy
    ...
    use PDF::Struct::Action :coerce;
    has PDF::Struct::Action::Destination $.OpenAction is entry(:&coerce);               #| (Optional; PDF 1.1) A value specifying a destination to be displayed or an action to be performed when the document is opened.
    ...
    has PDF::Struct::Resources $.Resources is entry;

    method cb-init {
        # vivify pages
	self<Type> //= PDF::DAO.coerce( :name<Catalog> );

        self<Pages> //= PDF::Struct::Pages.new(
            :dict{
