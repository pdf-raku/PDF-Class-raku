...
use PDF::Struct; # just to help rakudo
use PDF::DAO::Tie;
use PDF::DAO::Tie::Hash;

# AcroForm role - see PDF::Struct::Catalog - /AcroForm entry

role PDF::Struct::AcroForm
    does PDF::DAO::Tie::Hash {

    # see [PDF 1.7 TABLE 8.67 Entries in the interactive form dictionary]
    use PDF::Struct::Field :coerce;
    use PDF::DAO;

    has PDF::Struct::Field @.Fields is entry(:required, :&coerce);    #| (Required) An array of references to the document’s root fields (those with no ancestors in the field hierarchy).
    #| returns an inorder array of all desendant fields
    method fields returns Seq {
	my PDF::Struct::Field @fields;
	for $.Fields.keys {
