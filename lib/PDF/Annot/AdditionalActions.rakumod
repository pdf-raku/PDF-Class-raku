...
has PDF::Action $.E is entry(:alias<enter>);           #= (Optional; PDF 1.2) An action that is performed when the cursor enters the annotation’s active area.


has PDF::Action $.X is entry(:alias<exit>);            #= (Optional; PDF 1.2) An action that is performed when the cursor exits the annotation’s active area.

has PDF::Action $.D is entry(:alias<mouse-down>);      #= (Optional; PDF 1.2) An action that is performed when the mouse button is pressed inside the annotation’s active area.

has PDF::Action $.U is entry(:alias<mouse-up>);        #= (Optional; PDF 1.2) An action that is performed when the mouse button is released inside the annotation’s active area.
...
has PDF::Action $.Fo is entry(:alias<focus>);          #= (Optional; PDF 1.2; widget annotations only) An action that is performed when the annotation receives the input focus.

has PDF::Action $.Bl is entry(:alias<blur>);           #= (Optional; PDF 1.2; widget annotations only) (Uppercase B, lowercase L) An action that is performed when the annotation loses the input focus.

has PDF::Action $.PO is entry(:alias<page-open>);      #= (Optional; PDF 1.5) An action that is performed when the page containing the annotation is opened.
...
has PDF::Action $.PC is entry(:alias<page-close>);     #= (Optional; PDF 1.5) An action that is performed when the page containing the annotation is closed.
